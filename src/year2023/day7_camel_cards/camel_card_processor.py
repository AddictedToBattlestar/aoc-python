...
from src.year2023.day7_camel_cards.camel_card_calculator import get_ranking_of_hand, calculate_ranking_value
...
class CamelCardBid:
    def __init__(self, raw_hand_data, bid_amount):
        ...
        self.general_ranking, self.total_card_value = get_ranking_of_hand(raw_hand_data)

        logger.warning(
            f"{raw_hand_data},{bid_amount},{self.general_ranking.name},{self.general_ranking},{self.total_card_value}")
...
def calculate_total_winnings(ordered_camel_bid_list: list[CamelCardBid]):
    ...
    for camel_card_bid in ordered_camel_bid_list:
        ...
        logger.warning(f"Rank {current_bid_multiplier}, Hand: {camel_card_bid.raw_hand_data}, Bid: {camel_card_bid.bid_amount}, Total Bid: {total_winnings}, Details: ({camel_card_bid.general_ranking.name},{camel_card_bid.total_card_value})")
        current_bid_multiplier += 1
